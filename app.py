...
def clickable_kpi(label, value, colour="blue", key=None, subtext="Click to view details"):
    colour_map = {
        "blue": "#337ab7", "dark": "#173f67", "orange": "#f27c2c",
        "red": "#c90000", "green": "#70ad47"
    }
...
def ticket_dump_table(data):
    ...
    out["TAT"] = x["_TATDays"].apply(lambda v: f"{int(v)} days" if pd.notna(v) else "Not mapped")
    ...
    out["Reason for Pendency"] = col("Reason for pendency").replace("", "Reason not provided")
...
def get_kpi_ticket_subset(filtered_data, selected):
    ...
    if selected == "l2_mapped":
        return filtered_data[filtered_data["_IsL2"] & filtered_data["_TATHours"].notna()].copy()
    if selected == "l2_unmapped":
        return filtered_data[filtered_data["_IsL2"] & filtered_data["_TATHours"].isna()].copy()
    if selected == "l2_open":
        return filtered_data[filtered_data["_IsL2"] & ~filtered_data["_ClosedLike"]].copy()
...
def ticket_details_dialog(data, title, selected_key):
    ...
    with b:
        st.metric("TAT Mapped", f"{unique_ticket_count(data[data['_TATHours'].notna()]):,}")
    with c:
        st.metric("Open / Pending", f"{unique_ticket_count(data[~data['_ClosedLike']]):,}")
    with d:
        st.metric("TAT Breached", f"{unique_ticket_count(data[data['_TATStatus'].eq('Closed Beyond TAT')]):,}")

    st.markdown("#### Category / Sub-Category / TAT Breakup")
    breakup = tat_status_breakup_table(data)
    if not breakup.empty:
        st.dataframe(breakup, use_container_width=True, hide_index=True)

    st.markdown("#### Exact Ticket Dump")
    dump = ticket_dump_table(data)
    st.dataframe(dump, use_container_width=True, hide_index=True, height=500)
...
with q7:
    phone_l1 = l1a["l1"][clean_text(l1a["l1"]["Source"]).str.casefold().eq("phone")]
    kpi("PHONE TICKETS", f"{unique_ticket_count(phone_l1):,}", "green",
        "Customer called and raised the complaint")
with q8:
    chat_email_l1 = l1a["l1"][clean_text(l1a["l1"]["Source"]).str.casefold().isin(["chat","email"])]
    kpi("CHAT + EMAIL", f"{unique_ticket_count(chat_email_l1):,}", "orange",
        "Freshchat + customer email")
...
st.caption(
    "Phone = customer called; Chat = customer contacted us through Freshchat; "
    "Email = customer raised an email. Same-day closure is not labelled FCR because "
    "the raw dump does not contain Call ID/contact history."
)
